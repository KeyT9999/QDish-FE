      ...
      {isMobile ? (
        <div className="grid grid-cols-1 gap-4">
          {menuItems.map((item) => {
            ...
            return (
              <div key={itemId} className="bg-white border border-neutral-200/60 rounded-2xl p-4 shadow-sm flex gap-4 transition-all duration-200 hover:shadow-md">
                <img
                  src={item.imageUrl || 'https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=100&auto=format&fit=crop&q=60'}
                  alt={item.name}
                  className="w-24 h-24 object-cover rounded-xl border border-neutral-150 shrink-0"
                />
                <div className="flex-1 min-w-0 flex flex-col justify-between">
                  <div>
                    ...
                    <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-neutral-100 text-neutral-700 border border-neutral-200/50 mt-1">
                      {item.category}
                    </span>
                    
                    {item.nutrition && (
                      <div className="text-[10px] text-neutral-500 font-bold mt-1.5 flex flex-wrap items-center gap-1">
                        <span className="text-amber-600">🔥 {item.nutrition.calories} kcal</span>
