...
export const OwnerNotificationForm: React.FC = () => {
  ...
  const [targetType, setTargetType] = useState<NotificationTargetType>(NotificationTargetType.OWNER_RESTAURANTS);
  const [selectedRestaurantId, setSelectedRestaurantId] = useState('');
  const [targets, setTargets] = useState<OwnerNotificationTargets | null>(null);
  const [sending, setSending] = useState(false);

  useEffect(() => {
    notificationService.getOwnerTargets()
      .then(setTargets)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (needsSelector && selectedRestaurantId) {
        payload.targetIds = [selectedRestaurantId];
